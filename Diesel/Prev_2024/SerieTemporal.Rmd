...
for (i in 1:nrow(combinacoes)) {
  estado <- combinacoes[i, "Estado"][[1]]
  produto <- combinacoes[i, "Produto"][[1]]

  filtro <- BaseConsolidada[BaseConsolidada$Estado == estado & BaseConsolidada$Produto == produto, ]
  filtro_selecionado <- filtro[, c("Mes", "Media_Revenda")]

  nome_lista <- paste0(estado, "_", produto, "_historico")
...
for (nome_lista in names(previsoes_lista)) {
  ...
  dados_previsao <- data.frame(
    ...
    Nov_2023 = previsao$mean[1],
    Dez_2023 = previsao$mean[2],
    Jan_2024 = previsao$mean[3],
    Fev_2024 = previsao$mean[4],
    Mar_2024 = previsao$mean[5],
    Abr_2024 = previsao$mean[6],
    Mai_2024 = previsao$mean[7],
    Jun_2024 = previsao$mean[8],
    Jul_2024 = previsao$mean[9],
    Ago_2024 = previsao$mean[10],
    Set_2024 = previsao$mean[11],
    Out_2024 = previsao$mean[12],
    Nov_2024 = previsao$mean[13],
    Dez_2024 = previsao$mean[14]
  )
